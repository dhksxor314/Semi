...
public class createDao {
	...
	public void insertmetting(createDto dto) {
		String sql = "insert into Speech_Meeting(s_m, max_person, starttime, endtime, now_person, progression, min_person, image, region, description, approval, place_num"
				+ ", num, price, title, reporting_date, job, intro, message, Bank, accountholder, bankaccount, mem_num) values(?,?,?,?,?,?,?,?,?,?,?,?,seq_meeting_num.nextVal,?,?,?,?,?,?,?,?,?,?)";
		try {
	...
	public Vector endmetting(int mem_num) {
		...
		createDto list = null;
		String sql = "select s.num, s.title, p.place_name, s.now_person, s.approval, s.reporting_date, s.mem_num, s.max_person, m.name from speech_meeting s, place p, member m where s.place_num=p.place_num and s.mem_num=m.mem_num and s.progression='n' and s.mem_num="+mem_num;
		try {
			pool = DBConnectionMgr.getInstance();
			con = pool.getConnection();
			stmt = con.prepareStatement(sql);
	...
	public Vector getList(int mem_num) {
		...
		String sql = "select s.num, s.title, p.place_name, s.now_person, s.approval, s.reporting_date, s.mem_num, s.max_person, m.name from speech_meeting s, place p, member m where s.place_num=p.place_num and s.mem_num=m.mem_num and s.progression='y' and m.mem_num="+mem_num;
		try {
			pool = DBConnectionMgr.getInstance();
			con = pool.getConnection();
			stmt = con.prepareStatement(sql);
