...
작성자 - 이주연
수정날짜 - 2016/06/26
설명 - 회원의 강연, 모임에 대한 참가,개설 조회 및 취소 / 강연자의 개설내역  
 */
...
//활동내역_강연
		...
		sql ="select s.s_m, s.num, s.title, s.reporting_date, s.starttime, s.endtime, "
				...
				+ " where s.s_m='s' and sysdate() <= s.REPORTING_DATE and p.payment_option != -1 and p.mem_num="+mem_num;
...
//활동내역_모임
				...
				sql ="select s.s_m, s.num, s.title, s.reporting_date, s.starttime, s.endtime, s.image, pl.place_name, s.price, m.name"
						...
						+ " where s.s_m='m' and sysdate() <= s.REPORTING_DATE and p.payment_option != -1 and p.mem_num="+mem_num;
...
//개설한 모임을 출력(회원만이 모임을 개설할 수 있음)
		...
		//완료된 강연/모임 출력	
				public Vector<Participation_Dto> CompleteList(int mem_num){
					...
					try{
						...
						sql ="select s.s_m, s.title, s.reporting_date, s.starttime, s.endtime, s.image, pl.place_name, s.price "
								+ " from Speech_Meeting s"
								+ " join Participation p on (s.s_m = p.s_m and s.num = p.num)"
								+ " join Place pl on (s.place_num = pl.place_num)"
								+ " where s.REPORTING_DATE+1 < sysdate() and p.payment_option != -1 and p.mem_num="+mem_num;
	...
	//개설된 모임에 참가한 회원 정보를 개설자가 확인
				public Vector<MemberDto> ParticipantList(int mem_num, int num){
					...
					String sql = null;	
							...
							sql = "select p.mem_num, m.name, m.phone, m.email, s.now_person, s.max_person"
									...
									+ " where s.reporting_date > sysdate() and p.payment_option != -1 and s.mem_num="+mem_num+" and s.num="+num;
				...
				/*개설된 모임에 참가한 회원 정보를 개설자가 확인
				public void ParticipantList(int mem_num, int num){
						...
						String sql = null;	
							...
							sql = "select p.mem_num, m.name, m.phone, m.email, s.now_person, s.max_person"
									...
									+ " join Speech_Meeting s on (s.s_m = p.s_m and s.num = p.num)"
									+ " join Member m on (p.mem_num = m.mem_num)"
									+ " where s.reporting_date > sysdate() and s.mem_num="+mem_num+" and s.num="+num;
...
//강연자 마이페이지_완료된 개설내역 
				...
				public Vector<Participation_Dto> SpeakerCompleteList(){
					...
					try{
						...
						sql ="select s.title, s.reporting_date, s.now_person, s.image, pl.place_name, s.price "
								+ " from Speech_Meeting s"
								+ " join Place pl on (s.place_num = pl.place_num)"
								+ "	join Speaker sp on(sp.speaker_num = s.speaker_num)"
								+ " where sysdate() > s.REPORTING_DATE+1";
			...
			Vector<MemberDto> mem = new Vector<MemberDto>();	
				...
				String sql = null;	
					...
					sql = "select p.mem_num, m.name, m.phone, m.email, s.now_person, s.max_person"
							...
							+ " join Member m on (p.mem_num = m.mem_num)"
							+ " join Speaker sp on (s.speaker_num = sp.speaker_num) "
							+ " where p.payment_option != -1 and s.speaker_num="+speaker_num;
