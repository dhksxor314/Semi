...
강연자를 검색을 클릭하면 즐겨찾기에 추가하도록 하도록 수정필요.
 */
package dao;
...
//강연자 & 강연명 조회(즐겨찾기 개수 조회)
	public Vector<Bookmark_Dto> BookmarkList(int mem_num) {
		...
		sql = "select sp.name, sp.field, s.title, b.Bookmark_num, b.mem_num, s.reporting_date "
				...
				+ " where sysdate() <= s.REPORTING_DATE  and m.mem_num="+mem_num;
	...
	public boolean Overlap(int book_num){
			...
			String sql2 = "insert into Bookmark(mem_num, speaker_num) values(?,?)";
	...
	public Vector<Message_Dto> MessageWrite(Vector<Message_Dto> dto){	
		...
		String sql = null;	
			...
			sql = "insert into Message(mem_num, title, reportingdate, Confirmation, Bookmark_num) "
					+ " values (?,?,?,?,?)";			
		...
		for (int i = 0; i < dto.size(); i++) {
			...
			stmt.setDate(3, (java.sql.Date)dto.get(i).getReporting_date());
